...
function promiseTest() {
  ...
  it("Should be Some when the provided Promise resolves", async () =>
    expect(await Option.safe(test(false)).unwrapAsync()).to.equal("testing"));

  it("Should be None when the provided Promise rejects", async () =>
    expect(await Option.safe(test(true)).isNoneAsync()).to.be.true);
}
